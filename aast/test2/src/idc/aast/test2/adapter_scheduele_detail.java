...
static int dpi;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		for (int i=0 ;i<slots.size();i++) {
			...
			int start = Integer.parseInt(current.from);
			int end  = Integer.parseInt(current.to);
			position+=1;
			if((start) == position*2 || (start)==(position *2 -1 )  )
			{
				holder.course_name.setText(current.course);
				holder.course_code.setText(current.course_code);
				holder.room_data.setText(current.room_num);
				holder.name.setText(current.name);
				holder.kind.setText(current.type);
